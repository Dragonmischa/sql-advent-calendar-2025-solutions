...
SELECT
  subject,
  quiz_date,
  MIN(quiz_date) OVER (PARTITION BY subject) AS first_score_created_at,
  FIRST_VALUE(score) OVER (PARTITION BY subject ORDER BY quiz_date ASC) AS first_score,
  MAX(quiz_date) OVER (PARTITION BY subject) AS last_score_created_at,
